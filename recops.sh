...
get_regional_okta_aws_info () {
  ...
  echo "TW_REGION=${TW_REGION}" > .env-regional

  local AWS_OKTA_APP_ID="${AWS_OKTA_APP_IDS[$TW_REGION]}"
  export OKTA_APP_URL=https://thoughtworks.okta.com/home/amazon_aws/"${AWS_OKTA_APP_ID}"/272
  export AWS_ROLE_ARN=arn:aws:iam::"${AWS_ACCOUNT_IDS[$TW_REGION]}":role/federated-admin
}
...
function publish_candidate_zip {
  ...
  REGIONAL_S3_BUCKET=tw-joii-$(echo "${TW_REGION}" | awk '{print tolower($0)}')
...
function setup_user {
  ...
  USER_CREDS=$(dojo "./recops.sh _setup_creds")
...
case "${command}" in
  ...
  _create_user_creds)
      check_randomized
      INTERVIEW_CODE=$(cat interview_id.txt)
      aws iam create-access-key --user-name "interview-$INTERVIEW_CODE"
      ;;
  _setup_creds)
      randomize
